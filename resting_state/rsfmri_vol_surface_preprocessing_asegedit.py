...
def get_aparc_aseg(files):
    """Return the aseg.mgz file as no aparc+aseg.mgz in PING"""
    for name in files:
        if 'aparc+aseg.mgz' in name:
            return name
    raise ValueError('aseg.mgz not found')
...
def create_reg_workflow(name='registration'):
    ...
    register.connect(fssource, 'aseg', binarize, "in_file") #CHANGED
    ...
    register.connect(fssource, 'aseg', #CHANGED
...
def create_workflow(files,
    ...
    #######
    # Convert aparc to subject functional space #COMMENT OUT
    ...
    # wf.connect(collector, ('out', get_names, '_avgwf.txt'), #COMMENT OUT
    ...
    #            datasink, 'resting.parcellations.aparc.@avgwf') #COMMENT OUT
